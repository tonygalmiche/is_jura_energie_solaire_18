# -*- coding: utf-8 -*-
{
    "name"     : "Module Odoo 18 pour Jura Energie Solaire",
    "version"  : "18.0.0.3",
    "author"   : "InfoSaône",
    "category" : "InfoSaône",
    "description": """
Module Odoo 18 pour Jura Energie Solaire
===================================================
""",
    "maintainer" : "InfoSaône",
    "website"    : "http://www.infosaone.com",
    "depends"    : [
        "base",
        "purchase",
        "product",
        "project",
        "crm",
        "hr_timesheet",
        "l10n_fr_account",
        "web_chatter_position",
        "is_attachment_search",
        "is_gantt_view",
        "hr_holidays", # Congés
    ],
    "data" : [
        "security/is_suivi_temps_security.xml",
        "security/ir.model.access.csv",
        "data/purchase_selection_override.xml",
        "report/report_contrat_maintenance.xml",
        "report/report_protocole_maintenance.xml",
        "report/report_suivi_temps_semaine.xml",
        "report/report_layout.xml",
        "report/report_purchase_order.xml",
        "report/report_bon_intervention.xml",
        "views/base_menu_security.xml",
        "views/res_company_view.xml",
        "views/res_partner_view.xml",
        "views/res_users_view.xml",
        "views/product_view.xml",
        "views/project_view.xml",
        "views/is_sav_view.xml",
        "views/is_sav_intervention_view.xml",
        "views/is_sav_sous_statut_view.xml",
        "views/is_centrale_view.xml",
        "views/is_maintenance_view.xml",
        "views/is_maintenance_tache_view.xml",
        "views/is_suivi_temps_view.xml",
        "views/is_suivi_temps_semaine_view.xml",
        "views/is_preparation_paye_view.xml",
        "views/calendar_view.xml",
        "views/is_calendrier_equipe_view.xml",
        "views/crm_lead_view.xml",
        "views/is_documentation_view.xml",
        "views/purchase_order_view.xml",
        "views/centrale_map_template.xml",
        "views/menu.xml"
    ],
    'assets': {
        'web.assets_backend': [
            'is_jura_energie_solaire_18/static/src/scss/styles.scss',
            
        ],
    },
    "installable": True,
    "application": True,
    "license": "LGPL-3",
}
